  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0, flex: '0 0 auto' }}>
        ...
        <span style={{ flex: 1, minWidth: 0, fontSize: fs, fontWeight: 700, color: textCol, lineHeight: 1.2, ...nowrap }}>
          {reminder.title}
        </span>
        <button
          onClick={closeWindow}
          style={{ flexShrink: 0, display: 'inline-flex', alignItems: 'center', padding: 2, border: 'none', background: 'transparent', cursor: 'pointer', color: dimCol, borderRadius: 3, lineHeight: 1 }}
        >
          <X size={isize - 1} />
        </button>
      </div>
      ...
      {display.showContent && reminder.content && (
        <div style={{ fontSize: fsc, color: dimCol, lineHeight: 1.3, flex: '0 0 auto', ...nowrap }}>
          {reminder.content}
        </div>
      )}
      ...
      <div style={{ marginTop: 'auto', display: 'flex', flexDirection: 'column', gap: 4, flex: '0 0 auto' }}>
        ...
        <div style={{ display: 'flex', gap: 5, justifyContent: 'flex-end', alignItems: 'center' }}>
          <button
            ...
            style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: fsc, padding: '2px 7px', border: `1px solid ${snoozeB}`, borderRadius: 5, background: 'transparent', color: dimCol, cursor: 'pointer', whiteSpace: 'nowrap' }}
          >
            <Clock size={fsc - 1} />
          ...
          <button
            onClick={closeWindow}
            style={{ fontSize: fsc, padding: '2px 9px', border: 'none', borderRadius: 5, background: btnBg, color: '#fff', cursor: 'pointer', whiteSpace: 'nowrap' }}
          >
            關閉
          </button>
        </div>
